...
const createStudySession = async (req, res) => {
  try {
    const { subject, topic, startTime, endTime, sessionType, productivity, notes, breaksTaken, mood } = req.body;
    ...
    const duration = studySession.duration;
    const user = await User.findById(req.user.id);
    user.progressStats.totalStudyHours += Math.round(duration / 60 * 100) / 100; // Convert to hours
